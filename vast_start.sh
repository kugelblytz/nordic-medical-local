...
if [ "$LLM_PROVIDER" = "openai" ]; then
  if [ -z "${OPENAI_API_KEY:-}" ]; then
    ...
    echo "Export it in this shell before starting the experiment." >&2
    exit 1
  fi
  ...
  echo "== OpenAI development experiment =="
  echo "Model: $OPENAI_MODEL"
  echo "Reasoning: $OPENAI_REASONING"
  echo "Ollama is skipped in this mode."
  ...
  echo "ERROR: LLM_PROVIDER must be 'ollama' or 'openai'." >&2
  exit 1
fi
